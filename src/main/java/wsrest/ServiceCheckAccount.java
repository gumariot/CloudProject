...
public class ServiceCheckAccount {
	...
	public Response checkAccount(@PathParam("id") String id) {
		...
		int status = 200;
		if(id != null) {
			Response response = ClientBuilder.newClient().target("https://appcloud-195409.appspot.com/").path("accManager/" + id).request().get();
...
public class ServiceCheckAccount {
	...
	public Response checkAccount(@PathParam("id") String id) {
		...
		int status = 200;
		if(id != null) {
			Response response = ClientBuilder.newClient().target("https://appcloud-195409.appspot.com/").path("accManager/" + id).request(MediaType.APPLICATION_JSON).get();
			FormatedResponse data = response.readEntity(FormatedResponse.class);
			if(response.getStatus() != 200) {
				rep.setMessage(data.getMessage());
				rep.setDone(data.getDone());
